...
// ===== ADICIONE ESTAS FUNÇÕES AO SEU cardsProdutos.js =====

// Adicione essas variáveis no início do arquivo, junto com as outras
let termoBusca = "";
const inputBusca = document.querySelector('input[type="search"]');
const formBusca = document.querySelector('form[role="search"]');

// Adicione estes event listeners após os outros, na função DOMContentLoaded
function inicializarBuscaProdutos() {
  // Event listener para o input de busca (busca em tempo real)
  inputBusca.addEventListener("input", (e) => {
    termoBusca = e.target.value.toLowerCase().trim();
    aplicarFiltros(); // Usa a função existente para aplicar filtros
  });

  // Event listener para o formulário (quando pressiona Enter)
...
// Modifique a função aplicarFiltros existente para incluir a busca
function aplicarFiltros() {
  // ✅ SÓ APLICA FILTROS SE OS PRODUTOS JÁ FORAM CARREGADOS
  if (produtos.length === 0) {
    console.log("⚠️ Produtos ainda não foram carregados");
  ...
  const filtros = {
    ...
    cor: obterFiltrosPorCategoria("Cor"),
    categoria: obterCategoriaAtiva(),
    busca: termoBusca, // NOVA LINHA: adiciona o termo de busca
  };
  ...
  produtosFiltrados = produtos.filter((prod) => {
    ...
    const passaEsporte =
      filtros.esporte.length === 0 || filtros.esporte.includes(prod.esporte);
    const passaCor = filtros.cor.length === 0 || filtros.cor.includes(prod.cor);
    const passaCategoria =
      filtros.categoria.length === 0 || prod.categoria === filtros.categoria;
    ...
    const passaBusca =
      filtros.busca === "" ||
      prod.nome.toLowerCase().includes(filtros.busca) ||
      prod.marca.toLowerCase().includes(filtros.busca) ||
      prod.tipo.toLowerCase().includes(filtros.busca) ||
      prod.esporte.toLowerCase().includes(filtros.busca) ||
      prod.cor.toLowerCase().includes(filtros.busca) ||
      prod.descricao.toLowerCase().includes(filtros.busca);
    ...
    return (
      ...
      passaCategoria &&
      passaPreco &&
      passaBusca // NOVA LINHA: inclui o filtro de busca
    );
  });
  ...
  // Se há um termo de busca e nenhum resultado, exibe mensagem específica
  if (termoBusca && produtosFiltrados.length === 0) {
    exibirMensagemBuscaVazia();
  } else {
    aplicarOrdenacao();
  }
}

// Função para exibir mensagem quando a busca não retorna resultados
function exibirMensagemBuscaVazia() {
  container.innerHTML = `
    <div class="col-12">
      <div class="text-center py-5">
        <i class="bi bi-search fs-1 text-muted mb-3"></i>
        <h4 class="text-muted mb-3">Nenhum produto encontrado</h4>
        <p class="text-muted fs-5">Sua busca por "<strong>${termoBusca}</strong>" não retornou resultados.</p>
        <p class="text-muted">Tente:</p>
        <ul class="list-unstyled text-muted">
          <li>• Verificar a ortografia</li>
          <li>• Usar termos mais gerais</li>
          <li>• Buscar por marca, tipo de produto ou cor</li>
        </ul>
        <button class="btn btn-outline-danger mt-3" onclick="limparBusca()">
          <i class="bi bi-arrow-clockwise me-1"></i>Ver todos os produtos
        </button>
      </div>
    </div>
  `;
...
// Função para destacar o termo buscado nos resultados (opcional)
...
// Modifique a função renderizarProdutos para destacar os termos buscados
function renderizarProdutos(lista) {
  container.innerHTML = "";
  ...
  lista.forEach((prod) => {
    ...
    // Destaca o termo buscado se houver busca ativa
    const nomeDestacado = termoBusca
      ? destacarTermoBusca(prod.nome, termoBusca)
      : prod.nome;
    const descricaoDestacada = termoBusca
      ? destacarTermoBusca(prod.descricao, termoBusca)
      : prod.descricao;
    ...
    col.innerHTML = `
      ...
      prod.nome
        ...
        <div class="card-body d-flex flex-column justify-content-between">
          <div>
            <h5 class="card-title">${nomeDestacado}</h5>
            <p class="card-text text-muted small">${descricaoDestacada}</p>
            <div class="product-info">
              <p class="mb-1"><strong>Tamanho:</strong> ${prod.tamanho}</p>
              <p class="mb-1"><strong>Cor:</strong> ${prod.cor}</p>
              <p class="mb-2"><strong>Marca:</strong> ${prod.marca}</p>
...
// CHAME ESTA FUNÇÃO NO SEU DOMContentLoaded EXISTENTE
// Adicione esta linha após verificarStatusLogin();
